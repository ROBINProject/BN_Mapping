...
"""
from ctypes import windll
NETICA_LIB = u"C:/Users/Miguel/Documents/0 Versiones/2 Proyectos/Netica/lib/64 bit/Netica.dll"
...
#        self.ln = cdll.LoadLibrary("/Users/heijer/Downloads/Netica_API_504/lib/libnetica.so")
        # Versión rígida para mi máquina de llamada a NETICA
        self.ln = windll.LoadLibrary(NETICA_LIB)
    def newenv(self):
        """
        open environment
    ...
    def getnodeparents(self, node_p):
        # const nodelist_bn* GetNodeParents_bn (	const node_bn*  node )
        self.ln.GetNodeParents_bn.argtypes = [c_void_p,]
        self.ln.GetNodeParents_bn.restype = c_void_p
        return self.ln.GetNodeParents_bn(node_p, node_p)
    ...
    def getnodeprobs(self, node_p, parent_states):
        ...
        :return:
        """
        state_bn = array(parent_states, dtype='float32')
        # (	const node_bn*  node,   const state_bn*  parent_states )
        self.ln.SetNodeProbs_bn.argtypes = [c_void_p, ndpointer('float32', ndim=1, shape=(parent_states,), flags='C')]
        self.ln.SetNodeProbs_bn.restype = ndpointer('float32', ndim=1, shape=(state_bn,), flags='C')
        return self.ln.SetNodeProbs_bn(node_p, state_bn)
    def setnodeprobs(self, node_p, parent_states, probs):
        ...
        """
        state_bn = array(parent_states, dtype='float32')
        prob_bn = array(probs, dtype='float32')
        # (node_bn*  node,   const state_bn*  parent_states,   const prob_bn*  probs)
        self.ln.EnterNodeLikelihood_bn.argtypes = [c_void_p, ndpointer('float32', ndim=1, shape=(parent_states,),flags='C'),
                                                   ndpointer('float32', ndim=1, shape=(probs,),flags='C')]
        self.ln.EnterNodeLikelihood_bn.restype = None
        self.ln.EnterNodeLikelihood_bn(node_p, state_bn, prob_bn)
