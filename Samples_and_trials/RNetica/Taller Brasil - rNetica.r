...
# calcular la probabilidad a posteriori (believes)
# se requiere RNetica, hay que instalarlo desde un 
...
nd.ModisNPP <- NetworkFindNode(rb.integridad.brasil, "MODISNPP_MOD17a3_2010_rec_tif")
...
EnterFindings(rb.integridad.brasil,
              findings=c(Integrity="high", ModisNPP_MOD17A3_2010_rec_tif=25307))
...
lista.nodos <- list(nodos.datos[[1]], nodos.datos[[2]], nodos.datos[[4]],
                    ...
                    nodos.datos[[8]])
evidencias.brasil <- MemoryCaseStream(muestraTotalFixed)
memstream <- ReadFindings(lista.nodos, evidencias.brasil, "FIRST")
